...
def goal_path_resolve(new_node):
    ...
    if new_node.dist_to((shared.goal.x, shared.goal.y)) < shared.STEP_SIZE:
        ...
        if new_root_path_cost < shared.root_path_length:
            ...
            root_path = list(reversed(shared.root_path))
            searchNode = root_path[0]
            searchNodeIndex = 0
            shared.optimizedPath.append(root_path[0])
            while searchNodeIndex < (len(root_path) - 1):
                maxReachableIndex = searchNodeIndex + 1
                for i in range(searchNodeIndex + 1, len(root_path)):
                    new_node = root_path[i]
                    if obstacle_free(searchNode, (new_node.x, new_node.y)):
                        maxReachableIndex = i
                searchNodeIndex = maxReachableIndex
                searchNode = root_path[maxReachableIndex]
                shared.optimizedPath.append(searchNode)
...
def obstacle_free(node1, other):
    """ (TODO: Fix this function because this is not always good it just likely works)
    ...
    '''
    for obs in shared.obstacles:
        if (node1.x, node1.y) != other:
            dist = pnt2line((obs.x, obs.y), (node1.x, node1.y), other)
...
def pnt2line(pnt, start, end):
    ...
    #dont use this: nearest = add(nearest, start)
    return dist
